...
fn generate_q5_0_blocks(nb: usize) -> Vec<BlockQ5_0> {
    (0..nb).map(|i| {
        let mut qh = [0u8; 4];
        let mut qs = [0u8; 16];
        for j in 0..4 {
        ...
        BlockQ5_0 { d: f16::from_f32(1.0).to_bits(), qh, qs }
    }).collect()
}

fn generate_q5_1_blocks(nb: usize) -> Vec<BlockQ5_1> {
    (0..nb).map(|i| {
        let mut qh = [0u8; 4];
        let mut qs = [0u8; 16];
        for j in 0..4 {
        ...
        BlockQ5_1 { d: f16::from_f32(1.0).to_bits(), m: f16::from_f32(0.0).to_bits(), qh, qs }
    }).collect()
}
...
fn bench_fp32_neon(c: &mut Criterion) {
    let n = 1024 * 1024;
    let a = generate_fp32_data(n);
    let b = generate_fp32_data(n);

    c.bench_function("neon_fp32_1M", |bencher| {
        bencher.iter(|| unsafe {
            arm::vec_dot_fp32_neon(black_box(&a), black_box(&b))
        })
    });
}

fn bench_q4_0_neon(c: &mut Criterion) {
    let nb = 4096;
    let x = generate_q4_0_blocks(nb);
    let y = generate_q8_0_blocks(nb);
    let n = nb * QK4_0;

    c.bench_function("neon_q4_0_q8_0_131k", |bencher| {
        bencher.iter(|| unsafe {
            arm::vec_dot_q4_0_q8_0_neon(black_box(n), black_box(&x), black_box(&y))
        })
    });
}

fn bench_q8_0_neon(c: &mut Criterion) {
    let nb = 4096;
    let x = generate_q8_0_blocks(nb);
    let y = generate_q8_0_blocks(nb);
    let n = nb * QK4_0;

    c.bench_function("neon_q8_0_q8_0_131k", |bencher| {
        bencher.iter(|| unsafe {
            arm::vec_dot_q8_0_q8_0_neon(black_box(n), black_box(&x), black_box(&y))
        })
    });
}

fn bench_q4_1_neon(c: &mut Criterion) {
    let nb = 4096;
    let x = generate_q4_1_blocks(nb);
    let y = generate_q8_1_blocks(nb);
    let n = nb * QK4_0;

    c.bench_function("neon_q4_1_q8_1_131k", |bencher| {
        bencher.iter(|| unsafe {
            arm::vec_dot_q4_1_q8_1_neon(black_box(n), black_box(&x), black_box(&y))
        })
    });
}

fn bench_q5_0_neon(c: &mut Criterion) {
    let nb = 4096;
    let x = generate_q5_0_blocks(nb);
    let y = generate_q8_0_blocks(nb);
    let n = nb * QK4_0;

    c.bench_function("neon_q5_0_q8_0_131k", |bencher| {
        bencher.iter(|| unsafe {
            arm::vec_dot_q5_0_q8_0_neon(black_box(n), black_box(&x), black_box(&y))
        })
    });
}

fn bench_q5_1_neon(c: &mut Criterion) {
    let nb = 4096;
    let x = generate_q5_1_blocks(nb);
    let y = generate_q8_1_blocks(nb);
    let n = nb * QK4_0;

    c.bench_function("neon_q5_1_q8_1_131k", |bencher| {
        bencher.iter(|| unsafe {
            arm::vec_dot_q5_1_q8_1_neon(black_box(n), black_box(&x), black_box(&y))
        })
    });
}

fn bench_k_quant_neon(c: &mut Criterion) {
    let nb = 256;
    let n = nb * QK_K;
