# InferTrain Engine for JAX

## 概述

InferTrain 是一个用 C++ 和 Rust 编写的推训一体引擎，通过 FFI 暴露给 Python。
本插件为 JAX 版本，提供 PyTensor 类用于高性能张量运算。
...
JAX 的 Python 算子（如 + - * / 和 jnp.add）无法被覆盖。<br>
用户必须：<br>
&nbsp;&nbsp;1. 用 <code>jax_to_pytensor()</code> 将 jax.Array 转成 PyTensor<br>
&nbsp;&nbsp;2. 调用 PyTensor 的算子方法（如 a.add(b)）<br>
&nbsp;&nbsp;3. 用 <code>pytensor_to_jax()</code> 转回 jax.Array<br><br>

不要问为什么 a + b 不走引擎，因为 JAX 底层是 C++ 实现，Python 层无法覆盖。<br><br>
...
```
pip install infer_train_jax
```

## 使用方式

### 方式 1：从 jax.Array 转换（推荐）
```
...
jax_b = jnp.array([[5.0, 6.0], [7.0, 8.0]])

# 2. 转成 PyTensor
a = it.jax_to_pytensor(jax_a)
b = it.jax_to_pytensor(jax_b)

# 3. 使用 InferTrain 算子（必须显式调用）
c = a.add(b)          # ✅ 正确
# c = a + b           # ❌ 错误！走的是 JAX 原生，不是 InferTrain
...
__原因：JAX 核心是 C++ 实现，Python 层只是薄封装，无法有效覆盖。__
