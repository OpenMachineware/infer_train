...
fn main() {
    ...
    // Warm up CPU
    let warmup: Vec<u16> = (0..100000).map(|i| i as u16).collect();
    let mut warmup_out = vec![0.0f32; 100000];
    for _ in 0..100 {
        dequantize_row_f16(&warmup, &mut warmup_out);
    }

    let sizes = [1024, 4096, 16384, 65536];
    let runs = 20;  // Increased for stability
...
fn benchmark_f16(size: usize, runs: usize) {
    ...
    // Our quantize - use NEON on aarch64
    let start = Instant::now();
    for _ in 0..runs {
        #[cfg(target_arch = "aarch64")]
        quantize_row_f16_neon(&input, &mut output_f16);
    ...
    for _ in 0..runs {
        #[cfg(target_arch = "aarch64")]
        dequantize_row_f16_neon(&output_f16, &mut output_f32);
        #[cfg(not(target_arch = "aarch64"))]
    ...
    let start = Instant::now();
    for _ in 0..runs {
        dequantize_row_f16_llama(&output_f16, &mut output_f32_llama);
    }
...
fn benchmark_bf16(size: usize, runs: usize) {
    ...
    for _ in 0..runs {
        #[cfg(target_arch = "aarch64")]
        dequantize_row_bf16_neon(&output_bf16, &mut output_f32);
        #[cfg(not(target_arch = "aarch64"))]
    ...
    // llama.cpp dequantize (scalar reference) - use same data
    let mut output_f32_llama = vec![0.0f32; size];
    let start = Instant::now();
    for _ in 0..runs {
        dequantize_row_bf16_llama(&output_bf16, &mut output_f32_llama);
    }
    let llama_dequant_time = start.elapsed().as_secs_f64() / runs as f64;
    let llama_dequant_gbps = (size * 2) as f64 / llama_dequant_time / 1e9;
...
fn benchmark_q8_0(size: usize, runs: usize) {
    ...
    // Use same quantized data for both
    let q8_data = &output_q8;

    // Our dequantize FIRST - use NEON on aarch64
    let start = Instant::now();
    for _ in 0..runs {
        #[cfg(target_arch = "aarch64")]
        dequantize_row_q8_0_neon(q8_data, &mut output_f32);
        #[cfg(not(target_arch = "aarch64"))]
        dequantize_row_q8_0(q8_data, &mut output_f32);
    }
    let our_dequant_time = start.elapsed().as_secs_f64() / runs as f64;
    let our_dequant_gbps = (size * 4) as f64 / our_dequant_time / 1e9;

    // llama.cpp dequantize SECOND (scalar reference)
    let mut output_f32_llama = vec![0.0f32; size];
    let start = Instant::now();
    for _ in 0..runs {
        dequantize_row_q8_0_llama(q8_data, &mut output_f32_llama);
    }
    let llama_dequant_time = start.elapsed().as_secs_f64() / runs as f64;
...
fn benchmark_q4_0(size: usize, runs: usize) {
    ...
    // Our dequantize - use scalar (let compiler auto-vectorize)
    let start = Instant::now();
    for _ in 0..runs {
...
fn benchmark_q4_1(size: usize, runs: usize) {
    ...
    let start = Instant::now();
    for _ in 0..runs {
        dequantize_row_q4_1_llama(&output_q4, &mut output_f32_llama);
    }
    let llama_dequant_time = start.elapsed().as_secs_f64() / runs as f64;
    let llama_dequant_gbps = (size * 4) as f64 / llama_dequant_time / 1e9;
...
fn benchmark_q5_0(size: usize, runs: usize) {
    ...
    let start = Instant::now();
    for _ in 0..runs {
        dequantize_row_q5_0_llama(&output_q5, &mut output_f32_llama);
    }
    let llama_dequant_time = start.elapsed().as_secs_f64() / runs as f64;
    let llama_dequant_gbps = (size * 4) as f64 / llama_dequant_time / 1e9;
...
fn benchmark_iq4_nl(size: usize, runs: usize) {
    ...
    let start = Instant::now();
    for _ in 0..runs {
        dequantize_row_iq4_nl_llama(&output_iq4, &mut output_f32_llama);
    }
    let llama_dequant_time = start.elapsed().as_secs_f64() / runs as f64;
    let llama_dequant_gbps = (size * 4) as f64 / llama_dequant_time / 1e9;
