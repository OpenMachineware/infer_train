...
#
#   fused_matmul_add_bias_gpu(x, w, bias)
#       y[i,j] = sum_k x[i,k]*w[j,k] + bias[j]
#   fused_matmul_add_gpu(x, w, b)
#       y[i,j] = sum_k x[i,k]*w[j,k] + b[i,j]
#   fused_matmul_rms_norm_gpu(x, w, eps)
#       y[i,:] = rms_norm(x[i,:] @ w^T, eps)
#   fused_swiglu_matmul_gpu(g, u, w)
...
def _fused_matmul_add_kernel_f32(
    ...
    while i < n:
        ...
        while k < K_i:
            ...
            k += 1
        dst[unsafe_offset=i] = acc + b[unsafe_offset=i]
        i += stride
...
def _fused_swiglu_matmul_kernel_f32(
    ...
    while i < n:
        ...
        while f < F_i:
            acc += (
                _silu_f32(gate[unsafe_offset=row * F_i + f])
                * up[unsafe_offset=row * F_i + f]
                * w[unsafe_offset=col * F_i + f]
            )
            f += 1
...
def _fused_swiglu_matmul_kernel_f16(
    ...
    while i < n:
        ...
        var f = 0
        while f < F_i:
            var g = Float32(gate[unsafe_offset=row * F_i + f])
            var u = Float32(up[unsafe_offset=row * F_i + f])
            var wv = Float32(w[unsafe_offset=col * F_i + f])
...
def _fused_matmul_rms_norm_kernel_f32(
    ...
    while j < N_i:
        ...
        var k = 0
        while k < K_i:
            acc += x[unsafe_offset=row * K_i + k] * w[unsafe_offset=j * K_i + k]
            k += 1
        dst[unsafe_offset=row * N_i + j] = acc
        j += BLOCK
    barrier()
    ...
    j = tid
    while j < N_i:
        var v = dst[unsafe_offset=row * N_i + j]
        ss += v * v
        j += BLOCK
    ...
    j = tid
    while j < N_i:
        dst[unsafe_offset=row * N_i + j] *= inv
        j += BLOCK
...
def _fused_matmul_rms_norm_kernel_f16(
    ...
    while j < N_i:
        ...
        while k < K_i:
            acc += Float32(x[unsafe_offset=row * K_i + k]) * Float32(
                w[unsafe_offset=j * K_i + k]
            )
            k += 1
        dst[unsafe_offset=row * N_i + j] = Scalar[DType.float16](acc)
        j += BLOCK
    barrier()
    ...
    j = tid
    while j < N_i:
        var v = Float32(dst[unsafe_offset=row * N_i + j])
        ss += v * v
        j += BLOCK
    var ss_all = block_sum[block_size=BLOCK](ss)
    var inv = Float32(1.0) / sqrt(ss_all / Float32(N_i) + eps)
    ...
    j = tid
    while j < N_i:
        dst[unsafe_offset=row * N_i + j] = Scalar[DType.float16](
            Float32(dst[unsafe_offset=row * N_i + j]) * inv
        )
        j += BLOCK
...
def _fused_matmul_add_bias_gpu_launch[
    dtype: DType
](
    ctx: DeviceContext,
    x: Tensor[dtype, 2],
...
def _fused_matmul_add_gpu_launch[
    dtype: DType
](
    ctx: DeviceContext,
    x: Tensor[dtype, 2],
...
def _fused_matmul_rms_norm_gpu_launch[
    dtype: DType
](
...
def _fused_swiglu_matmul_gpu_launch[
    dtype: DType
](
    ctx: DeviceContext,
    gate: Tensor[dtype, 2],
...
def fused_matmul_add_bias_gpu[
    dtype: DType
](x: Tensor[dtype, 2], w: Tensor[dtype, 2], bias: Tensor[dtype, 1]) -> Tensor[
    dtype, 2
]:
...
def fused_matmul_add_gpu[
    dtype: DType
](x: Tensor[dtype, 2], w: Tensor[dtype, 2], b: Tensor[dtype, 2]) -> Tensor[
    dtype, 2
]:
...
def fused_swiglu_matmul_gpu[
    dtype: DType
](gate: Tensor[dtype, 2], up: Tensor[dtype, 2], w: Tensor[dtype, 2]) -> Tensor[
    dtype, 2
]:
