...
# IQ2_XXS × Q8_K kernel - NEON SIMD optimized version
...
# IQ2_XXS grid as static constant (256 entries, stored in .rodata)
# Each entry is uint64 encoding 8 int8 values (e.g., 0x08, 0x19, 0x2b)
comptime IQ2XXS_GRID: Array[UInt64, 256] = [
...
def vec_dot_iq2xxs_q8k_neon(
    x: Pointer[UInt8, MutUntrackedOrigin],
    y: Pointer[UInt8, MutUntrackedOrigin],
    nb: Int,
) -> Float32:
    """IQ2_XXS × Q8_K dot product - NEON SIMD optimized version.

    Block layout (66 bytes):
    - d: FP16 scale (2 bytes)
    - qs[32]: uint16 array (64 bytes)
      Each 4 uint16 values encode:
      - Low 8 bits of each uint16 = grid index (4 per 32-element sub-block)
      - High bits encode scale and signs for the sub-block
    """
    # Get reference to static grid and signs table
    ref grid_ref = global_constant[IQ2XXS_GRID]()
    ...
    for i in range(nb):
        ...
        for _ in range(4):  # 4 iterations of 2 blocks each
            ...
            # Load 8 uint16 = 4 uint32 for two sub-blocks
            # Layout: [u16_0..7] = [aux32[0..3]]
            var qs_bytes = x.unsafe_load[width=16](offset=q2_ptr)
            q2_ptr += 16

            # Convert bytes to 4 uint32 values
            var aux32_0 = UInt32(qs_bytes[0]) | (UInt32(qs_bytes[1]) << 8) | (UInt32(qs_bytes[2]) << 16) | (UInt32(qs_bytes[3]) << 24)
            var aux32_1 = UInt32(qs_bytes[4]) | (UInt32(qs_bytes[5]) << 8) | (UInt32(qs_bytes[6]) << 16) | (UInt32(qs_bytes[7]) << 24)
            var aux32_2 = UInt32(qs_bytes[8]) | (UInt32(qs_bytes[9]) << 8) | (UInt32(qs_bytes[10]) << 16) | (UInt32(qs_bytes[11]) << 24)
            var aux32_3 = UInt32(qs_bytes[12]) | (UInt32(qs_bytes[13]) << 8) | (UInt32(qs_bytes[14]) << 16) | (UInt32(qs_bytes[15]) << 24)

            # Grid indices: aux8[0..3] and aux8[8..11]
            # aux8[i] is the i-th byte, which is the low 8 bits of a uint16
            # For aux32_0 and aux32_1 (first sub-block): aux8[0..3] are the low bytes
            # For aux32_2 and aux32_3 (second sub-block): aux8[8..11] = aux32_2/3 low bytes

            # Load grid values (each grid entry is uint64 = 8 int8 values)
            var q2u_0 = combine_s8_from_u64(
                grid.unsafe_load[width=1](offset=Int(aux32_0 & 0xFF)),
                grid.unsafe_load[width=1](offset=Int((aux32_0 >> 8) & 0xFF)),
            )
            var q2u_1 = combine_s8_from_u64(
                grid.unsafe_load[width=1](offset=Int((aux32_0 >> 16) & 0xFF)),
                grid.unsafe_load[width=1](offset=Int((aux32_0 >> 24) & 0xFF)),
            )
            var q2u_2 = combine_s8_from_u64(
                grid.unsafe_load[width=1](offset=Int(aux32_2 & 0xFF)),
                grid.unsafe_load[width=1](offset=Int((aux32_2 >> 8) & 0xFF)),
            )
            var q2u_3 = combine_s8_from_u64(
                grid.unsafe_load[width=1](offset=Int((aux32_2 >> 16) & 0xFF)),
                grid.unsafe_load[width=1](offset=Int((aux32_2 >> 24) & 0xFF)),
            )

            # Load signs from keven_signs_q2xs (same as IQ3_XXS)
            # signs64 is a uint64 pointer, each entry is 8 bytes of signs
            # Signs are in aux32_1 and aux32_3
            ...
            var q2s_2 = combine_s8_from_u64(
                signs64.unsafe_load[width=1](offset=Int((aux32_3 >> 0) & 127)),
                signs64.unsafe_load[width=1](offset=Int((aux32_3 >> 7) & 127)),
            )
            var q2s_3 = combine_s8_from_u64(
                signs64.unsafe_load[width=1](offset=Int((aux32_3 >> 14) & 127)),
                signs64.unsafe_load[width=1](offset=Int((aux32_3 >> 21) & 127)),
            )
            ...
            # SDOT dot products
            var p1 = neon_sdot(neon_sdot(SIMD[DType.int32, 4](0, 0, 0, 0), q2u_0, q8b.val0), q2u_1, q8b.val1)
            var p2 = neon_sdot(neon_sdot(SIMD[DType.int32, 4](0, 0, 0, 0), q2u_2, q8b.val2), q2u_3, q8b.val3)

            # Scale: 0.5 + (aux32 >> 28)
            var ls1 = Float32(0.5) + Float32(aux32_1 >> 28)
            var ls2 = Float32(0.5) + Float32(aux32_3 >> 28)

            sumf1 += Float32(neon_addv(p1)) * ls1
            sumf2 += Float32(neon_addv(p2)) * ls2

        # Final scale: multiply by d and 0.25 (from llama.cpp)
