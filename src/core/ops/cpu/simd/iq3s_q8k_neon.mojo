...
# IQ3_S grid as static constant (stored in .rodata)
comptime IQ3S_GRID: Array[UInt32, 512] = [
...
# Efficiently pack 4 uint32 values into a SIMD vector (like llama.cpp's ggml_vld1q_u32)
# Packs pairs into uint64 then constructs vector - more efficient than SIMD constructor
@always_inline
def pack_u32x4(w: UInt32, x: UInt32, y: UInt32, z: UInt32) -> SIMD[DType.uint32, 4]:
    # Pack pairs: w|x -> uint64, y|z -> uint64
    var low64 = UInt64(w) | (UInt64(x) << 32)
    var high64 = UInt64(y) | (UInt64(z) << 32)
    # Construct uint64x2 then bitcast to uint32x4
    var vec64 = SIMD[DType.uint64, 2](low64, high64)
    return bitcast[DType.uint32, 4](vec64)
...
def vec_dot_iq3s_q8k_neon(
    ...
    """
    # Get reference to static grid (stored in .rodata, no allocation)
    ref grid_ref = global_constant[IQ3S_GRID]()
    # Get raw pointer to bypass bounds checking
    ...
    for i in range(nb):
        ...
        # Pre-process scales (like llama.cpp)
        # Load 4 bytes, then expand to 8 scales
        var scales_raw = x.unsafe_load[width=4](offset=x_base + 106)
        var scales32_0 = UInt32(scales_raw[0]) | (UInt32(scales_raw[1]) << 8) | (UInt32(scales_raw[2]) << 16) | (UInt32(scales_raw[3]) << 24)
        var scales32_1 = ((scales32_0 >> 4) & 0x0f0f0f0f) << 1 | 0x01010101
        scales32_0 = (scales32_0 & 0x0f0f0f0f) << 1 | 0x01010101
        ...
        for ib32 in range(0, 8, 2):
            ...
            # Load 8 grid values directly, using inline packing
            # Each grid value is loaded and packed efficiently
            var w0 = UInt64(grid.unsafe_load[width=1](offset=Int(indices0[0])))
            var x0 = UInt64(grid.unsafe_load[width=1](offset=Int(indices0[1])))
            var y0 = UInt64(grid.unsafe_load[width=1](offset=Int(indices0[2])))
            var z0 = UInt64(grid.unsafe_load[width=1](offset=Int(indices0[3])))
            var aux0_0 = bitcast[DType.uint32, 4](SIMD[DType.uint64, 2](w0 | (x0 << 32), y0 | (z0 << 32)))

            var w1 = UInt64(grid.unsafe_load[width=1](offset=Int(indices0[4])))
            var x1 = UInt64(grid.unsafe_load[width=1](offset=Int(indices0[5])))
            var y1 = UInt64(grid.unsafe_load[width=1](offset=Int(indices0[6])))
            var z1 = UInt64(grid.unsafe_load[width=1](offset=Int(indices0[7])))
            var aux0_1 = bitcast[DType.uint32, 4](SIMD[DType.uint64, 2](w1 | (x1 << 32), y1 | (z1 << 32)))
            ...
            var w2 = UInt64(grid.unsafe_load[width=1](offset=Int(indices1[0])))
            var x2 = UInt64(grid.unsafe_load[width=1](offset=Int(indices1[1])))
            var y2 = UInt64(grid.unsafe_load[width=1](offset=Int(indices1[2])))
            var z2 = UInt64(grid.unsafe_load[width=1](offset=Int(indices1[3])))
            var aux1_0 = bitcast[DType.uint32, 4](SIMD[DType.uint64, 2](w2 | (x2 << 32), y2 | (z2 << 32)))

            var w3 = UInt64(grid.unsafe_load[width=1](offset=Int(indices1[4])))
            var x3 = UInt64(grid.unsafe_load[width=1](offset=Int(indices1[5])))
            var y3 = UInt64(grid.unsafe_load[width=1](offset=Int(indices1[6])))
            var z3 = UInt64(grid.unsafe_load[width=1](offset=Int(indices1[7])))
            var aux1_1 = bitcast[DType.uint32, 4](SIMD[DType.uint64, 2](w3 | (x3 << 32), y3 | (z3 << 32)))

            # Load signs (like llama.cpp: signs[0], signs[1], signs[2], signs[3] as uint16)
            var signs_offset = signs_base + ib32 * 4
            var sign_byte0 = x.unsafe_load[width=1](offset=signs_offset)
            var sign_byte1 = x.unsafe_load[width=1](offset=signs_offset + 1)
            var sign_byte2 = x.unsafe_load[width=1](offset=signs_offset + 2)
            var sign_byte3 = x.unsafe_load[width=1](offset=signs_offset + 3)
            var sign_byte4 = x.unsafe_load[width=1](offset=signs_offset + 4)
            var sign_byte5 = x.unsafe_load[width=1](offset=signs_offset + 5)
            var sign_byte6 = x.unsafe_load[width=1](offset=signs_offset + 6)
            var sign_byte7 = x.unsafe_load[width=1](offset=signs_offset + 7)
            # Combine bytes into uint16 values (little-endian)
            var sign0 = UInt32(sign_byte0) | (UInt32(sign_byte1) << 8)
            var sign1 = UInt32(sign_byte2) | (UInt32(sign_byte3) << 8)
            var sign2 = UInt32(sign_byte4) | (UInt32(sign_byte5) << 8)
            var sign3 = UInt32(sign_byte6) | (UInt32(sign_byte7) << 8)

            # Sign processing using TBL
            # First pair of sign bytes (for aux0_0 and aux0_1)
            var sign_pair0 = sign0 | (sign1 << 16)
            var vs0 = neon_vreinterpretq_u8_u32(neon_vdupq_n_u32(sign_pair0))
            ...
            var sign_pair1 = sign2 | (sign3 << 16)
            var vs1 = neon_vreinterpretq_u8_u32(neon_vdupq_n_u32(sign_pair1))
            ...
            # Use pre-processed scales (like llama.cpp)
            # scales8[ib32/2] -> scales32_0 byte at position (ib32/2)
            # scales8[ib32/2+4] -> scales32_1 byte at position (ib32/2)
            var scale_idx = ib32 // 2
            var ls0 = Int((scales32_0 >> UInt32(scale_idx * 8)) & 0xFF)
            var ls1 = Int((scales32_1 >> UInt32(scale_idx * 8)) & 0xFF)
