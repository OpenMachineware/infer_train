...
pub fn cast<T: DType + Send + Sync, U: DType + Send + Sync>(input: &Tensor<T>) -> Tensor<U> {
    let data: Vec<U> = input.data()
        .par_iter()
        .map(|&x| U::from_f32(x.to_f32()))
        .collect();
...
pub fn to_bf16<T: DType + Send + Sync>(input: &Tensor<T>) -> Tensor<half::bf16> {
    use half::bf16;
    cast::<T, bf16>(input)
}
