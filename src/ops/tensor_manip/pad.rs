...
pub fn pad<T: DType + Send + Sync>(
    ...
    for o in 0..outer {
        for i in 0..shape[shape.len() - 1] {
            ...
            let dst_idx = (o + pad[0]) * new_shape[shape.len() - 1] + (i + pad[pad.len() - 2]);
...
pub fn pad_backward<T: DType>(
    ...
    for o in 0..outer {
        for i in 0..original_shape[original_shape.len() - 1] {
            let src_idx = (o + pad[0]) * inner_stride + (i + pad[pad.len() - 2]);
...
pub fn quantized_pad(
    ...
    let shape = input.shape();
    assert_eq!(pad.len(), shape.len() * 2, "quantized_pad: pad length must be 2 * rank");
...
impl<T: DType + Send + Sync> Operator<T> for PadOp {
    fn name(&self) -> &'static str { "pad" }
    fn forward(&self, inputs: &[&Tensor<T>], attrs: &OpAttrs) -> Tensor<T> {
        assert_eq!(inputs.len(), 1);
        let padding = attrs.get_int_list("pad")
...
impl Operator<i8> for QuantizedPadOp {
    fn name(&self) -> &'static str { "quantized_pad" }
