...
pub fn concat<T: DType + Send + Sync>(
    ...
    for input in inputs {
        assert_eq!(
            input.shape().len(),
            rank,
            "concat: all inputs must have same rank"
        );
...
pub fn quantized_concat(inputs: &[&Tensor<i8>], dim: usize) -> Tensor<i8> {
    assert!(
        !inputs.is_empty(),
        "quantized_concat: at least one input required"
    );
    ...
    for input in inputs {
        assert_eq!(
            input.shape().len(),
            rank,
            "quantized_concat: all inputs must have same rank"
        );
        for (i, &d) in input.shape().iter().enumerate() {
            if i != dim {
                assert_eq!(
                    d, first_shape[i],
                    "quantized_concat: shape mismatch at dim {}",
                    i
                );
            }
        }
...
pub fn quantized_concat_backward(
    ...
    for &shape in input_shapes {
        ...
        result.push(Tensor::<i8>::new_quantized(
            grad_slice, shape, scale, zero_point,
        ));
        offset += len;
    }
    result
