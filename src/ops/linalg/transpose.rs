...
pub fn transpose_backward<T: DType>(
...
pub fn quantized_transpose(a: &Tensor<i8>) -> Tensor<i8> {
    let shape = a.shape();
    assert!(shape.len() >= 2, "quantized_transpose requires at least 2D tensor");
...
impl Operator<i8> for QuantizedTransposeOp {
    fn name(&self) -> &'static str { "quantized_transpose" }
    fn forward(&self, inputs: &[&Tensor<i8>], _attrs: &OpAttrs) -> Tensor<i8> {
        assert_eq!(inputs.len(), 1);
        quantized_transpose(inputs[0])
    }
    fn backward(&self, grad: &Tensor<i8>, _inputs: &[&Tensor<i8>], _attrs: &OpAttrs) -> Vec<Tensor<i8>> {
