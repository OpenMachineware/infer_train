...
pub fn log_softmax<T: DType + Send + Sync>(a: &Tensor<T>, dim: usize) -> Tensor<T> {
    let sm = softmax(a, dim);
    let data: Vec<T> = sm.data()
        .par_iter()
        .map(|&x| T::from_f32(x.to_f32().ln()))
        .collect();
...
impl<T: DType + Send + Sync> Operator<T> for LogSoftmaxOp {
    fn name(&self) -> &'static str { "log_softmax" }
...
impl Operator<i8> for QuantizedLogSoftmaxOp {
    fn name(&self) -> &'static str { "quantized_log_softmax" }
