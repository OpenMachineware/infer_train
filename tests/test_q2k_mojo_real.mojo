# Mojo Q2_K test - same data and method as llama.cpp test
from src.core.ops.cpu.simd.q2k_q8k_dot import vec_dot_q2_k_q8_k
...
def main():
    ...
    var q2_block_size = 84
    var q8_block_size = 292

    # Allocate test data
    var q2_mem = unsafe_alloc[UInt8](nb * q2_block_size, alignment=64)
    var q8_mem = unsafe_alloc[UInt8](nb * q8_block_size, alignment=64)
    var q2_ptr = Pointer[UInt8, MutUntrackedOrigin](q2_mem)
    var q8_ptr = Pointer[UInt8, MutUntrackedOrigin](q8_mem)

    # Initialize data (same as C test)
    for i in range(nb):
        var block_offset = i * q2_block_size
        # Initialize scales
        for j in range(16):
            var val = UInt8((j + 1) | (j << 4))
            q2_ptr.unsafe_offset(block_offset + j).unsafe_store(val)
        # Initialize qs
        for j in range(64):
            q2_ptr.unsafe_offset(block_offset + 16 + j).unsafe_store(UInt8(j % 256))
        # Initialize d and dmin (use FP16 like actual model)
        q2_ptr.unsafe_offset(block_offset + 80).unsafe_bitcast[Scalar[DType.float16]]().unsafe_store(Scalar[DType.float16](1.0))
        q2_ptr.unsafe_offset(block_offset + 82).unsafe_bitcast[Scalar[DType.float16]]().unsafe_store(Scalar[DType.float16](0.0))
    ...
    for _ in range(warmup):
        ...
        for i in range(nb):
            sum += vec_dot_q2_k_q8_k(
                q2_ptr.unsafe_offset(i * q2_block_size),
